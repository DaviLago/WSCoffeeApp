...
public class ArticleController {
	...
	public ResponseEntity<List<ArticleModel>> findAllByTheme(@PathVariable Theme theme) {
		return new ResponseEntity<>(articleService.findAllByTheme(theme), HttpStatus.OK);
	}
	
	@GetMapping("/{articleid}")
    public ResponseEntity<ArticleModel> findById(@PathVariable String articleid) {
		Optional<ArticleModel> oArticle = articleService.findById(articleid);
		if(oArticle.isPresent())
			return new ResponseEntity<>(oArticle.get(), HttpStatus.OK);
